...
## Fixes
- Antigravity protocol fidelity: make 9Router's upstream Antigravity traffic match the installed IDE client. Uses the production `cloudcode-pa.googleapis.com` host (not the `daily-`/sandbox dev host), derives the `User-Agent` from the installed IDE version instead of a hardcoded string, requires the real `cloudaicompanionProject` id, and keeps internal headers (e.g. `x-request-source`) off upstream requests. (A request-level `metadata` field is intentionally not injected — the AG API rejects it with `INVALID_ARGUMENT` — so IDE-version fidelity rides on the User-Agent.) The OAuth client secret is read from `ANTIGRAVITY_OAUTH_CLIENT_SECRET` (no committed fallback). Adopted from upstream PR #1615 (antigravity-only commits; the unrelated caveman/wenyan, DeepSeek-1M, CI and build-script changes from that branch were not taken).
...
- Antigravity MITM tool calls: include `thoughtSignature` on returned `functionCall` parts. Antigravity/Gemini 3+ rejects tool calls without it (`Function call is missing a thought_signature...`), so OpenAI/Claude tool-call streams now carry the synthetic Antigravity thinking signature when translated back.
- Antigravity MITM → Claude: cap forwarded `max_tokens` at 64000. Antigravity IDE 2.0.4 can send `generationConfig.maxOutputTokens=65536`; Anthropic rejects that (`max_tokens: 65536 > 64000`), so 9Router now enforces its provider-safe ceiling before sending Claude-format requests.
...
- MITM tool cards: mark the models a tool sends automatically (Kiro `auto`/`simple-task`, Copilot `gpt-5-mini`/`gpt-5.4-nano`/`claude-haiku-4.5`) with a `*` + hover tooltip, and state that any unmapped model is sent to the original provider instead of being re-routed. Also correct the hosts-file note: 9Router adds/removes those entries automatically when DNS is toggled (no manual edit needed).
- MITM diagnostics: opt-in request/response dumps via `MITM_FILE_LOG=1` (the intercepted child always runs `NODE_ENV=production`, so the dev-only file log never fired in a normal install). Dumped request/response headers are now masked (auth tokens, cookies, API keys), and any model that has no mapping and falls through to the tool's real upstream is logged live (`⚠️ passthrough <tool>: model="…" has no mapping`) — making the Kiro-`auto` class of silent misrouting visible for any MITM tool.
